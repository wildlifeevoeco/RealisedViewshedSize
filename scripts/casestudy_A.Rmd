---
title: "Calculating the Realised Viewshed Size for Case Study A"
---
...
package.list=c("readr", "dplyr", "janitor", "data.table", "tidyr", "stats",
               ...
               "gammit", "gratia", "gridExtra", "ggpubr")
...
total_mod <- gam(detection ~ s(x_distance, grid_dist, bs='gp') #Gaussian process spline on physical locations in front of camera (equivalent to kriging to allow for interpolation inbetween transects we jogged on)
                + te(shrub, cover) #tensor product spline (interaction) for vegetation height and cover
                + brand + temp + velocity + seconds_since #categories temperature, and velocity as standard GLM variables 
                + s(unique_id, bs='fs'), # factor smooth for random effect, allows variable curvature for each level 
                 data = GAMM.df, family = 'binomial', correlation = corAR1(form = ~   1|detection_datetime))
...
#unique_id
unique_id <- rep("RM-99-99", times=840)
#new df of all preds.
...
base::sum(heat_df$mean) #mean ECA prediction 
base::sum(heat_df$lower) #lower credible interval 
base::sum(heat_df$upper) #upper credible interval
```
...
shrub_es$prediction <- predict_gamm(total_mod, shrub_es, type = "link", 
                          se.fit = TRUE, re_form =  NA, 
                          exclude = c("seconds_since"))   #rerun w.o exclude 
...
#scale values to be % deviation from mean 
# mean shrub cover value = 62 %
shrub_r$percent <- abs((shrub_r$mean - (shrub_r$mean[shrub_r$shrub==62]))/(shrub_r$mean[shrub_r$shrub==62])) *100
    
#mean influence of change in unit value 
abs(base::mean(diff(shrub_r$mean))) 
(base::max(shrub_r$mean) - base::min(shrub_r$mean))

s <- ggplot(data = shrub_r, aes(x=shrub,y=mean)) +
  geom_ribbon(aes(ymin=lower, ymax=upper), colour = "grey", alpha =0.3) +
  geom_line(colour = "black", linewidth = 1) +
  xlab("Percent shrub cover") + ylab("") + ylim(0,150) +
...
#scale values to be % deviation from mean 
# mean horizontal cover value = 79.6 % 
cover_r$percent <- abs((cover_r$mean - (cover_r$mean[cover_r$cover==79]))/(cover_r$mean[cover_r$cover==79])) *100
    
#mean influence of change in unit value 
abs(base::mean(diff(cover_r$mean)))
(base::max(cover_r$mean) - base::min(cover_r$mean))

c <- ggplot(data = cover_r, aes(x=cover,y=mean)) +
  geom_ribbon(aes(ymin=lower, ymax=upper), colour = "grey", alpha =0.3) +
  geom_line(colour = "black", linewidth = 1) +
  xlab("Percent horizontal cover of vegetation") + ylab("") + ylim(0,150) +
  theme(axis.title=element_text(size=14, colour = "black")) +
  theme(axis.text=element_text(size=14, colour = "black")) +
  theme_minimal()
```
...
#scale values to be % deviation from mean 
# mean temp = 1.5 celcius during surveys
temp_r$percent <- abs((temp_r$mean - (temp_r$mean[temp_r$temp==2]))/(temp_r$mean[temp_r$temp==2])) *100
    
#mean influence of change in unit value 
abs(base::mean(diff(temp_r$mean)))
(base::max(temp_r$mean) - base::min(temp_r$mean))

t <- ggplot(data = temp_r, aes(x=temp,y=mean)) +
  geom_ribbon(aes(ymin=lower, ymax=upper), colour = "grey", alpha =0.3) +
  geom_line(colour = "black", linewidth = 1) +
  xlab("Ambient temperature °C") + ylab("") + ylim(0,150) +
  theme(axis.title=element_text(size=14, colour = "black")) +
  theme(axis.text=element_text(size=14, colour = "black")) +
  theme_minimal()
```
...
#scale values to be % deviation from mean 
# mean velocity = 2 m/s jogs 
velocity_r$percent <- abs((velocity_r$mean - (velocity_r$mean[velocity_r$velocity==2]))/(velocity_r$mean[velocity_r$velocity==2])) *100
    
#mean influence of change in unit value 
abs(base::mean(diff(velocity_r$mean)))
(base::max(velocity_r$mean) - base::min(velocity_r$mean))

v <- ggplot(data = velocity_r, aes(x=velocity,y=mean)) +
  geom_ribbon(aes(ymin=lower, ymax=upper), colour = "grey", alpha =0.3) +
  geom_line(colour = "black", linewidth = 1) +
  xlab("Velocity (m/s) of jog") + ylab("") + ylim(0,150) +
  theme(axis.title=element_text(size=14, colour = "black")) +
  theme(axis.text=element_text(size=14, colour = "black")) +
  theme_minimal()
```
...
annotate_figure(figure, left = text_grob(bquote("Realised Viewshed Size" ~(m^2)), 
...
c_16_9 <- gamm(detection ~ s(x_distance, grid_dist, bs='gp') 
                + te(shrub, cover)
                + brand + as.factor(seconds_since) + temp + velocity
                + s(unique_id, bs='fs'), 
                family = 'binomial', data = GAMM.df[GAMM.df$unique_id != 'RM-16-9',], correlation = corAR1(form = ~ 1|detection_datetime))

c_14_9 <- gamm(detection ~ s(x_distance, grid_dist, bs='gp') 
                + te(shrub, cover)
                + brand + as.factor(seconds_since) + temp + velocity
                + s(unique_id, bs='fs'), 
                family = 'binomial', data = GAMM.df[GAMM.df$unique_id != 'RM-14-9',], correlation = corAR1(form = ~ 1|detection_datetime))

c_19_9 <- gamm(detection ~ s(x_distance, grid_dist, bs='gp') 
                + te(shrub, cover)
                + brand + as.factor(seconds_since) + temp + velocity
                + s(unique_id, bs='fs'), 
                family = 'binomial', data = GAMM.df[GAMM.df$unique_id != 'RM-19-9',], correlation = corAR1(form = ~ 1|detection_datetime))

c_30_9 <- gamm(detection ~ s(x_distance, grid_dist, bs='gp') 
                + te(shrub, cover)
                + brand + as.factor(seconds_since) + temp + velocity
                + s(unique_id, bs='fs'), 
                family = 'binomial', data = GAMM.df[GAMM.df$unique_id != 'RM-30-9',], correlation = corAR1(form = ~ 1|detection_datetime))

c_4_9 <- gamm(detection ~ s(x_distance, grid_dist, bs='gp') 
                + te(shrub, cover)
                + brand + as.factor(seconds_since) + temp + velocity
                + s(unique_id, bs='fs'), 
                family = 'binomial', data = GAMM.df[GAMM.df$unique_id != 'RM-4-9',], correlation = corAR1(form = ~ 1|detection_datetime))

c_6_9 <- gamm(detection ~ s(x_distance, grid_dist, bs='gp') 
                + te(shrub, cover)
                + brand + as.factor(seconds_since) + temp + velocity
                + s(unique_id, bs='fs'), 
                family = 'binomial', data = GAMM.df[GAMM.df$unique_id != 'RM-6-9',], correlation = corAR1(form = ~ 1|detection_datetime))

c_8_9 <- gamm(detection ~ s(x_distance, grid_dist, bs='gp') 
                + te(shrub, cover)
                + brand + as.factor(seconds_since) + temp + velocity
                + s(unique_id, bs='fs'), 
                family = 'binomial', data = GAMM.df[GAMM.df$unique_id != 'RM-8-9',], correlation = corAR1(form = ~ 1|detection_datetime))

c_14_1 <- gamm(detection ~ s(x_distance, grid_dist, bs='gp') 
                + te(shrub, cover)
                + brand + as.factor(seconds_since) + temp + velocity
                + s(unique_id, bs='fs'), 
                family = 'binomial', data = GAMM.df[GAMM.df$unique_id != 'RM-14-1',], correlation = corAR1(form = ~ 1|detection_datetime))

c_14_2 <- gamm(detection ~ s(x_distance, grid_dist, bs='gp') 
                + te(shrub, cover)
                + brand + as.factor(seconds_since) + temp + velocity
                + s(unique_id, bs='fs'), 
                family = 'binomial', data = GAMM.df[GAMM.df$unique_id != 'RM-14-2',], correlation = corAR1(form = ~ 1|detection_datetime))

c_14_3 <- gamm(detection ~ s(x_distance, grid_dist, bs='gp') 
                + te(shrub, cover)
                + brand + as.factor(seconds_since) + temp + velocity
                + s(unique_id, bs='fs'), 
                family = 'binomial', data = GAMM.df[GAMM.df$unique_id != 'RM-14-3',], correlation = corAR1(form = ~ 1|detection_datetime))

c_14_4 <- gamm(detection ~ s(x_distance, grid_dist, bs='gp') 
                + te(shrub, cover)
                + brand + as.factor(seconds_since) + temp + velocity
                + s(unique_id, bs='fs'), 
                family = 'binomial', data = GAMM.df[GAMM.df$unique_id != 'RM-14-4',], correlation = corAR1(form = ~ 1|detection_datetime))

c_14_5 <- gamm(detection ~ s(x_distance, grid_dist, bs='gp') 
                + te(shrub, cover)
                + brand + as.factor(seconds_since) + temp + velocity
                + s(unique_id, bs='fs'), 
                family = 'binomial', data = GAMM.df[GAMM.df$unique_id != 'RM-14-5',], correlation = corAR1(form = ~ 1|detection_datetime))

c_14_7 <- gamm(detection ~ s(x_distance, grid_dist, bs='gp') 
                + te(shrub, cover)
                + brand + as.factor(seconds_since) + temp + velocity
                + s(unique_id, bs='fs'), 
                family = 'binomial', data = GAMM.df[GAMM.df$unique_id != 'RM-14-7',], correlation = corAR1(form = ~ 1|detection_datetime))

c_14_8 <- gamm(detection ~ s(x_distance, grid_dist, bs='gp') 
                + te(shrub, cover)
                + brand + as.factor(seconds_since) + temp + velocity
                + s(unique_id, bs='fs'), 
                family = 'binomial', data = GAMM.df[GAMM.df$unique_id != 'RM-14-8',], correlation = corAR1(form = ~ 1|detection_datetime))

c_16_1 <- gamm(detection ~ s(x_distance, grid_dist, bs='gp') 
                + te(shrub, cover)
                + brand + as.factor(seconds_since) + temp + velocity
                + s(unique_id, bs='fs'), 
                family = 'binomial', data = GAMM.df[GAMM.df$unique_id != 'RM-16-1',], correlation = corAR1(form = ~ 1|detection_datetime))

c_16_6 <- gamm(detection ~ s(x_distance, grid_dist, bs='gp') 
                + te(shrub, cover)
                + brand + as.factor(seconds_since) + temp + velocity
                + s(unique_id, bs='fs'), 
                family = 'binomial', data = GAMM.df[GAMM.df$unique_id != 'RM-16-6',], correlation = corAR1(form = ~ 1|detection_datetime))

c_16_7 <- gamm(detection ~ s(x_distance, grid_dist, bs='gp') 
                + te(shrub, cover)
                + brand + as.factor(seconds_since) + temp + velocity
                + s(unique_id, bs='fs'), 
                family = 'binomial', data = GAMM.df[GAMM.df$unique_id != 'RM-16-7',], correlation = corAR1(form = ~ 1|detection_datetime))

c_19_4 <- gamm(detection ~ s(x_distance, grid_dist, bs='gp') 
                + te(shrub, cover)
                + brand + as.factor(seconds_since) + temp + velocity
                + s(unique_id, bs='fs'), 
                family = 'binomial', data = GAMM.df[GAMM.df$unique_id != 'RM-19-4',], correlation = corAR1(form = ~ 1|detection_datetime))

c_19_6 <- gamm(detection ~ s(x_distance, grid_dist, bs='gp') 
                + te(shrub, cover)
                + brand + as.factor(seconds_since) + temp + velocity
                + s(unique_id, bs='fs'), 
                family = 'binomial', data = GAMM.df[GAMM.df$unique_id != 'RM-19-6',], correlation = corAR1(form = ~ 1|detection_datetime))

c_30_1 <- gamm(detection ~ s(x_distance, grid_dist, bs='gp') 
                + te(shrub, cover)
                + brand + as.factor(seconds_since) + temp + velocity
                + s(unique_id, bs='fs'), 
                family = 'binomial', data = GAMM.df[GAMM.df$unique_id != 'RM-30-1',], correlation = corAR1(form = ~ 1|detection_datetime))

c_30_8 <- gamm(detection ~ s(x_distance, grid_dist, bs='gp') 
                + te(shrub, cover)
                + brand + as.factor(seconds_since) + temp + velocity
                + s(unique_id, bs='fs'), 
                family = 'binomial', data = GAMM.df[GAMM.df$unique_id != 'RM-30-8',], correlation = corAR1(form = ~ 1|detection_datetime))

c_4_1 <- gamm(detection ~ s(x_distance, grid_dist, bs='gp') 
                + te(shrub, cover)
                + brand + as.factor(seconds_since) + temp + velocity
                + s(unique_id, bs='fs'), 
                family = 'binomial', data = GAMM.df[GAMM.df$unique_id != 'RM-4-1',], correlation = corAR1(form = ~ 1|detection_datetime))

c_4_2 <- gamm(detection ~ s(x_distance, grid_dist, bs='gp') 
                + te(shrub, cover)
                + brand + as.factor(seconds_since) + temp + velocity
                + s(unique_id, bs='fs'), 
                family = 'binomial', data = GAMM.df[GAMM.df$unique_id != 'RM-4-2',], correlation = corAR1(form = ~ 1|detection_datetime))

c_4_4 <- gamm(detection ~ s(x_distance, grid_dist, bs='gp') 
                + te(shrub, cover)
                + brand + as.factor(seconds_since) + temp + velocity
                + s(unique_id, bs='fs'), 
                family = 'binomial', data = GAMM.df[GAMM.df$unique_id != 'RM-4-4',], correlation = corAR1(form = ~ 1|detection_datetime))

c_4_6 <- gamm(detection ~ s(x_distance, grid_dist, bs='gp') 
                + te(shrub, cover)
                + brand + as.factor(seconds_since) + temp + velocity
                + s(unique_id, bs='fs'), 
                family = 'binomial', data = GAMM.df[GAMM.df$unique_id != 'RM-4-6',], correlation = corAR1(form = ~ 1|detection_datetime))

c_4_8 <- gamm(detection ~ s(x_distance, grid_dist, bs='gp') 
                + te(shrub, cover)
                + brand + as.factor(seconds_since) + temp + velocity
                + s(unique_id, bs='fs'), 
                family = 'binomial', data = GAMM.df[GAMM.df$unique_id != 'RM-4-8',], correlation = corAR1(form = ~ 1|detection_datetime))

c_6_1 <- gamm(detection ~ s(x_distance, grid_dist, bs='gp') 
                + te(shrub, cover)
                + brand + as.factor(seconds_since) + temp + velocity
                + s(unique_id, bs='fs'), 
                family = 'binomial', data = GAMM.df[GAMM.df$unique_id != 'RM-6-1',], correlation = corAR1(form = ~ 1|detection_datetime))

c_6_3 <- gamm(detection ~ s(x_distance, grid_dist, bs='gp') 
                + te(shrub, cover)
                + brand + as.factor(seconds_since) + temp + velocity
                + s(unique_id, bs='fs'), 
                family = 'binomial', data = GAMM.df[GAMM.df$unique_id != 'RM-6-3',], correlation = corAR1(form = ~ 1|detection_datetime))

c_6_5 <- gamm(detection ~ s(x_distance, grid_dist, bs='gp') 
                + te(shrub, cover)
                + brand + as.factor(seconds_since) + temp + velocity
                + s(unique_id, bs='fs'), 
                family = 'binomial', data = GAMM.df[GAMM.df$unique_id != 'RM-6-5',], correlation = corAR1(form = ~ 1|detection_datetime))

c_6_6 <- gamm(detection ~ s(x_distance, grid_dist, bs='gp') 
                + te(shrub, cover)
                + brand + as.factor(seconds_since) + temp + velocity
                + s(unique_id, bs='fs'), 
                family = 'binomial', data = GAMM.df[GAMM.df$unique_id != 'RM-6-6',], correlation = corAR1(form = ~ 1|detection_datetime))

c_6_7 <- gamm(detection ~ s(x_distance, grid_dist, bs='gp') 
                + te(shrub, cover)
                + brand + as.factor(seconds_since) + temp + velocity
                + s(unique_id, bs='fs'), 
                family = 'binomial', data = GAMM.df[GAMM.df$unique_id != 'RM-6-7',], correlation = corAR1(form = ~ 1|detection_datetime))

c_6_8 <- gamm(detection ~ s(x_distance, grid_dist, bs='gp') 
                + te(shrub, cover)
                + brand + as.factor(seconds_since) + temp + velocity
                + s(unique_id, bs='fs'), 
                family = 'binomial', data = GAMM.df[GAMM.df$unique_id != 'RM-6-8',], correlation = corAR1(form = ~ 1|detection_datetime))

c_8_1 <- gamm(detection ~ s(x_distance, grid_dist, bs='gp') 
                + te(shrub, cover)
                + brand + as.factor(seconds_since) + temp + velocity
                + s(unique_id, bs='fs'), 
                family = 'binomial', data = GAMM.df[GAMM.df$unique_id != 'RM-8-1',], correlation = corAR1(form = ~ 1|detection_datetime))

c_8_4 <- gamm(detection ~ s(x_distance, grid_dist, bs='gp') 
                + te(shrub, cover)
                + brand + as.factor(seconds_since) + temp + velocity
                + s(unique_id, bs='fs'), 
                family = 'binomial', data = GAMM.df[GAMM.df$unique_id != 'RM-8-4',], correlation = corAR1(form = ~ 1|detection_datetime))

c_8_5 <- gamm(detection ~ s(x_distance, grid_dist, bs='gp') 
                + te(shrub, cover)
                + brand + as.factor(seconds_since) + temp + velocity
                + s(unique_id, bs='fs'), 
                family = 'binomial', data = GAMM.df[GAMM.df$unique_id != 'RM-8-5',], correlation = corAR1(form = ~ 1|detection_datetime))

c_8_7 <- gamm(detection ~ s(x_distance, grid_dist, bs='gp') 
                + te(shrub, cover)
                + brand + as.factor(seconds_since) + temp + velocity
                + s(unique_id, bs='fs'), 
                family = 'binomial', data = GAMM.df[GAMM.df$unique_id != 'RM-8-7',], correlation = corAR1(form = ~ 1|detection_datetime))

c_8_8 <- gamm(detection ~ s(x_distance, grid_dist, bs='gp') 
                + te(shrub, cover)
                + brand + as.factor(seconds_since) + temp + velocity
                + s(unique_id, bs='fs'), 
                family = 'binomial', data = GAMM.df[GAMM.df$unique_id != 'RM-8-8',], correlation = corAR1(form = ~ 1|detection_datetime))
...
#need to account for hand motions too. re load transect start times 


GAMM.df <- arrange(GAMM.df, unique_id, detection_datetime)

#make detection logical for loop 
GAMM.df$hit <- ifelse(GAMM.df$detection == 1, TRUE, FALSE)

# Create a new column to store elapsed time
GAMM.df$elapsed_time <- NA

# Get the unique IDs
unique_ids <- unique(GAMM.df$unique_id)

# Loop over each unique ID
for (id in unique_ids) {
  # Subset the data for the current ID
  subset_data <- GAMM.df[GAMM.df$unique_id == id, ]
  
  # Variable to store the most recent TRUE timestamp
  last_true_time <- NA
  
  # Loop over rows for this ID
  for (i in 1:nrow(subset_data)) {
    if (subset_data$hit[i]) {
      # Update the most recent TRUE timestamp
      last_true_time <- subset_data$detection_datetime[i]
      GAMM.df$elapsed_time[GAMM.df$unique_id == id & GAMM.df$detection_datetime == subset_data$detection_datetime[i]] <- 0
    } else if (!is.na(last_true_time)) {
      # Calculate elapsed time since the most recent TRUE value
      GAMM.df$elapsed_time[GAMM.df$unique_id == id & GAMM.df$detection_datetime == subset_data$detection_datetime[i]] <-
        as.numeric(difftime(subset_data$detection_datetime[i], last_true_time, units = "secs"))
    }
  }
}

#convert NA's to a high value (i.e., no capture in a long time)
GAMM.df$elapsed_time <- replace_na(GAMM.df$elapsed_time, 60)

#log elapsed time for a spline 
#first very small value for 0
GAMM.df$elapsed_time <- (GAMM.df$elapsed_time + 0.00001)
GAMM.df$log_hit <- log(GAMM.df$elapsed_time)
